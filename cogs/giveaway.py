...
class GiveawayView(discord.ui.View):
    ...
    async def join_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        button.label = await self.custom_response("giveaway.message.button", interaction, participants=len(self.participants))
        await interaction.response.edit_message(view=self)  # type: ignore
...
class Giveaway(commands.Cog):
    ...
    async def end_giveaway(self, ctx: Context, message_id: int, channel_id: int, right_now: bool = False):
        ...
        try:
            ...
            if winners:
                response = await self.custom_response("giveaway.end.success", ctx, winners=", ".join(winners))
    ...
    @app_commands.rename(
        winners="gw_specs-args-winners-name", duration="gw_specs-args-duration-name",
    ...
    async def giveaway(self, ctx: Context, duration: str, winners: str = None, *, prize: str = None):
        try:
            duration_dt = datetime.now() + timedelta(seconds=helpers.convert_time(duration))
        ...
        message = await ctx.send("giveaway.start.response", view=view, prize=prize, winners=winners_count, ends=FormatDateTime(end_time, "R"))
    ...
    @giveaway.command(name="end", description="gw_end-description", usage="gw_end-usage")
    @app_commands.rename(
        message_id="gw_end-args-message_id-name"
    )
    @app_commands.describe(
        message_id="gw_end-args-message_id-description"
    )
